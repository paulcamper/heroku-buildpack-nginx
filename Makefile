...
build-heroku-20:
	...
	@docker run -v $(shell pwd):/buildpack --rm -it -e "STACK=heroku-20" -e "NGINX_VERSION=1.14.0" -e "PCRE_VERSION=8.42" -e "HEADERS_MORE_VERSION=0.33" -w /buildpack heroku/heroku:20-build scripts/build_nginx /buildpack/bin/nginx-heroku-20
